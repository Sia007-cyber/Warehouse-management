...
public class PermissionTab extends Tab {
    ...
    private BorderPane buildContent() {
        ...
        TableColumn<Permission, String> warehouseCol = new TableColumn<>("انبار");
        ...
        TableColumn<Permission, String> itemCol = new TableColumn<>("کالا");
        ...
        TableColumn<Permission, Number> priceCol = new TableColumn<>("قیمت واحد");
        ...
        TableColumn<Permission, String> statusCol = new TableColumn<>("وضعیت");
        ...
        int r = 0;
        form.add(new Label("نوع مجوز:"), 0, r);
        form.add(typeCombo, 1, r++);
        form.add(new Label("انبار:"), 0, r);
        form.add(warehouseCombo, 1, r++);
        form.add(new Label("کالا:"), 0, r);
        form.add(itemCombo, 1, r++);
        form.add(new Label("تعداد:"), 0, r);
        form.add(quantityField, 1, r++);
        form.add(new Label("قیمت واحد:"), 0, r);
        form.add(unitPriceField, 1, r++);
        form.add(new Label("عنوان:"), 0, r);
        form.add(titleField, 1, r++);
        form.add(new Label("توضیحات:"), 0, r);
        form.add(descField, 1, r++);
        form.add(new Label("تاریخ:"), 0, r);
        ...
        Button issueBtn = new Button("صدور مجوز");
        issueBtn.setOnAction(e -> onIssue());
        Button confirmBtn = new Button("تأیید مجوز انتخاب‌شده");
        confirmBtn.setOnAction(e -> onConfirm());
        Button refreshBtn = new Button("بازخوانی جدول");
        refreshBtn.setOnAction(e -> refresh());
        HBox buttons = new HBox(6, issueBtn, confirmBtn, refreshBtn);

        balanceLabel.setStyle("-fx-font-weight: bold;");

        VBox side = new VBox(10, new Label("صدور مجوز جدید"), form, buttons,
    ...
    private void onIssue() {
        try {
            ...
            if (type == null || warehouse == null || item == null) {
                throw new IllegalArgumentException("نوع، انبار و کالا باید انتخاب شوند.");
            }
            ...
            Dialogs.error("تعداد و قیمت واحد باید عدد باشند.");
        } catch (Exception e) {
    ...
    private void onConfirm() {
        ...
        if (selected == null) {
            Dialogs.error("ابتدا یک مجوز را از جدول انتخاب کنید.");
            return;
        }
        if (selected.getStatus() != PermissionStatus.ISSUED) {
            Dialogs.error("این مجوز قبلاً تأیید شده است.");
            return;
        }
        try {
    ...
    private void refreshBalance() {
        try {
            balanceLabel.setText("موجودی نقدی فعلی: " + ctx.accountingService.getBalance());
        } catch (SQLException e) {
            Dialogs.error("خطا در خواندن موجودی نقدی: " + e.getMessage());
        }
    }
    ...
    private void refresh() {
        try {
            ...
            Dialogs.error("خطا در بارگذاری مجوزها: " + e.getMessage());
        }
    }
}
