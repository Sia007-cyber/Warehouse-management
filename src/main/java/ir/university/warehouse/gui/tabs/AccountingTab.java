...
public class AccountingTab extends Tab {
    ...
    private BorderPane buildContent() {
        ...
        amountField.setPromptText("مبلغ");
        Button depositBtn = new Button("افزودن موجودی نقدی (شارژ حساب)");
        depositBtn.setOnAction(e -> onDeposit());

        HBox depositBox = new HBox(8, new Label("مبلغ:"), amountField, depositBtn);
        depositBox.setPadding(new Insets(10, 0, 10, 0));

        TableColumn<Transaction, Number> idCol = new TableColumn<>("شناسه");
        idCol.setCellValueFactory(c -> new javafx.beans.property.SimpleIntegerProperty(c.getValue().getTransactionId()));
        TableColumn<Transaction, Number> permCol = new TableColumn<>("شناسه مجوز");
        permCol.setCellValueFactory(c -> new javafx.beans.property.SimpleIntegerProperty(c.getValue().getPermissionId()));
        TableColumn<Transaction, String> typeCol = new TableColumn<>("نوع");
        typeCol.setCellValueFactory(c -> new javafx.beans.property.SimpleStringProperty(c.getValue().getType().toString()));
        TableColumn<Transaction, Number> amountCol = new TableColumn<>("مبلغ");
        ...
        VBox top = new VBox(6, balanceLabel, depositBox, new Label("تاریخچه تراکنش‌های مالی:"));
        root.setTop(top);
    ...
    private void onDeposit() {
        try {
            ...
            if (amount <= 0) throw new IllegalArgumentException("مبلغ باید مثبت باشد.");
            ctx.accountingService.deposit(amount);
            amountField.clear();
            refresh();
        } catch (NumberFormatException e) {
            Dialogs.error("مبلغ باید عدد باشد.");
        } catch (Exception e) {
    ...
    private void refresh() {
        try {
            ...
            Dialogs.error("خطا در بارگذاری اطلاعات حسابداری: " + e.getMessage());
        }
    }
}
